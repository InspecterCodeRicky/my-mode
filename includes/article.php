<?php
    class Article {
        public function fetch_all($tab) {
            ...
            $query = $pdo->prepare("SELECT * FROM $tab ORDER BY CAST(date_create AS DATE) ASC");
            $query -> execute();
            return $query->fetchAll(\PDO::FETCH_ASSOC);
        }

        public function fetch_rand($nbr) {
            global $pdo;

            $query = $pdo->prepare("SELECT * FROM articles ORDER BY RAND() LIMIT $nbr ");
            $query -> execute();
            return $query->fetchAll();
        }
        public function get_article_by_keyAccess($id,$keyAccess, $tab) {
            global $pdo ;
        ...
        public function fetch_keyWord($key) {
            ...
            $query = $pdo->prepare("SELECT * FROM articles WHERE title LIKE ? || content LIKE ? || category LIKE ? || author LIKE ? || date LIKE ?");
